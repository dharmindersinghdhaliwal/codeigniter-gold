...
<div class="single-channel-page" id="content-wrapper">
				...
				<div class="col-md-8">
					<div class="single-video-left">
						...
						<?php
							...
							<div id="zoomvideoplayer" class="zoomvideoplayer auto-init" data-source="<?= BASE_URL ?>upload_data/video/<?= $video->video ?>" data-sourceogg="<?= BASE_URL ?>upload_data/video/<?= $video->video ?>" data-options='{autoplay: "off",settings_suggestedQuality: "hd720",downloadlink:"off",responsive_ratio: "0.565",disable_downloadbtn:"on"}' <?= $cover ?> <?= $logo ?> data-ad_array='[{"source":"<?= $advt ?>","type":"video","ad_type":"video","ad_time":"<?= $ad_time ?>","ad_skip_delay":"5"}]'>
     			 			</div>
						</div>
	...
	$(function() {
		...
		var video = $('video')[0];
		// forward and back video 5 sec with left and right arrow
		...
		function onTrackedVideoFrame(currentTime, duration){
			...
			if((advt_timing!=undefined) ||(advt_timing!='')){
				...
				//track advertisement video timing 
				$("#the-media-con-zoomvideoplayer_ad_0 video").on("timeupdate",function(event){

					//when advt video stopped update watched time
					if (this.paused) {
			    		$.ajax({
							url: '<?= BASE_URL ?>video_play/save_advt_impression',
							type: 'POST',data: {'video_id':<?= $video->id ?>,'advt_id':<?= $advt_id ?>,'advt_watchtime':this.currentTime}
						}).done(function(res) {
							console.log(res);
						}).fail(function() {
