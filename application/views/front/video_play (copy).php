<!--<link rel="stylesheet" type="text/css" href="<?= BASE_ASSET ?>video-player/build/mediaelementplayer.css" />-->
...
<div class="single-channel-page" id="content-wrapper">
	<div class="container-fluid pb-0">
		<div class="video-block section-padding">
			<div class="row">
				...
				<!--<video id="player1" preload="auto" width="640" height="360"	style="max-width:100%;"	src="<?= BASE_URL ?>upload_data/video/<?= $video->video ?>"	<?php if($video->video_thumbnail != ''){ ?>	poster="<?= BASE_URL ?>upload_data/video/thumbnail/<?= $video->video_thumbnail ?>"	<?php } ?>	controls playsinline webkit-playsinline></video>-->
						...
						?>
							<div id="zoomvideoplayer" class="zoomvideoplayer auto-init" data-source="<?= BASE_URL ?>upload_data/video/<?= $video->video ?>" data-options='{autoplay: "off",settings_suggestedQuality: "hd720",downloadlink:"off",responsive_ratio: "0.565"}' <?= $cover ?>     <?= $logo ?> data-ad_array='[{"source":"<?= $advt ?>","type":"video","ad_type":"video","ad_time":"<?= $ad_time ?>","ad_skip_delay":"5"}]'>
     			 			</div>
						</div>
							...
							<?php foreach (db_get_all_data('video_comment',array('video_id'=>$video->id),false,'datetime DESC',15) as $video_comment) {
								...
								if($comment_toEdit!=$video_comment->id){
									...
									if(db_get_count_data('users',array('id'=>$video_comment->user_id)) != 0){ ?>
										...
										<?php
											$default_channel = db_get_row_data('channel',array('user_id'=>$video_comment->user_id,'is_default'=>1));
										?>
										<div class="single-video-author box mb-3">
											...
											<strong>
												<?php 
												$channel_url = isset($default_channel->id) ? $default_channel->id : $video->channel_id;;
												?>
											...
											<div class="like-dislike-panel">
												...
												<div class="viewReply-panel">
													<?php 
														$replies  = db_get_all_data('reply_comment',array('comment_id'=>$video_comment->id));
													?>
	...
	<!--<script src="<?= BASE_ASSET ?>video-player/build/mediaelement-and-player.js"></script>-->
	<script>
		/*$(function () {
			$('#player1').mediaelementplayer({
				// startVolume: 0.8,// loop: false,	// enableAutosize: true,// features: ypause','current','progress','duration','tracks','volume','fullscreen'],
				// alwaysShowControls: false,// iPadUseNativeControls: false,// iPhoneUseNativeControls: false,
				// AndroidUseNativeControls: false,	// alwaysShowHours: false,// showTimecodeFrameCount: false,// enableKeyboard: true,
				// pauseOtherPlayers: true,	// stretching: 'auto',// pluginPath: '<?= BASE_ASSET ?>video-player/build/',// keyActions: []
			});
		});*/
	</script>	
